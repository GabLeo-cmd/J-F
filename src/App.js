...
// ── Cotações ──────────────────────────────────────────────────────
const MOEDAS_LISTA = [
  { code: 'USD', nome: 'Dólar Americano',    flag: '🇺🇸' },
  { code: 'EUR', nome: 'Euro',               flag: '🇪🇺' },
  { code: 'GBP', nome: 'Libra Esterlina',    flag: '🇬🇧' },
  { code: 'CNY', nome: 'Yuan Chinês',        flag: '🇨🇳' },
  { code: 'ARS', nome: 'Peso Argentino',     flag: '🇦🇷' },
  { code: 'CLP', nome: 'Peso Chileno',       flag: '🇨🇱' },
  { code: 'COP', nome: 'Peso Colombiano',    flag: '🇨🇴' },
  { code: 'PEN', nome: 'Sol Peruano',        flag: '🇵🇪' },
  { code: 'BOB', nome: 'Boliviano',          flag: '🇧🇴' },
  { code: 'PYG', nome: 'Guarani',            flag: '🇵🇾' },
  { code: 'UYU', nome: 'Peso Uruguaio',      flag: '🇺🇾' },
  { code: 'GYD', nome: 'Dólar da Guiana',   flag: '🇬🇾' },
  { code: 'JPY', nome: 'Iene Japonês',       flag: '🇯🇵' },
  { code: 'CHF', nome: 'Franco Suíço',       flag: '🇨🇭' },
  { code: 'CAD', nome: 'Dólar Canadense',    flag: '🇨🇦' },
  { code: 'AUD', nome: 'Dólar Australiano',  flag: '🇦🇺' },
  { code: 'MXN', nome: 'Peso Mexicano',      flag: '🇲🇽' },
  { code: 'ZAR', nome: 'Rand Sul-Africano',  flag: '🇿🇦' },
];

function CotacoesBanner() {
  ...
  useEffect(() => {
    async function buscarCotacoes() {
      try {
        ...
        for (const m of MOEDAS_LISTA) {
          if (rates[m.code]) {
            const brl = (1 / rates[m.code]);
            const decimais = brl < 0.01 ? 6 : brl < 1 ? 4 : 4;
            itens.push({
              label: `${m.flag} ${m.code}/BRL`,
              nome:  m.nome,
              valor: `R$ ${brl.toLocaleString('pt-BR', { minimumFractionDigits: decimais, maximumFractionDigits: decimais })}`,
            });
          }
        }
        ...
        // Venezuela via Yadio
        try {
          const ves = await fetch('https://yadio.io/expon/VES');
          const vd  = await ves.json();
          if (vd?.BRL) {
            itens.push({
              label: '🇻🇪 VES/BRL',
              nome:  'Bolívar Venezuelano',
              valor: `R$ ${Number(vd.BRL).toLocaleString('pt-BR', { minimumFractionDigits: 6, maximumFractionDigits: 6 })}`,
            });
          }
        } catch (_) {}
        ...
        console.warn('[Cotações] erro:', e.message);
        setLoading(false);
      }
    }
    buscarCotacoes();
    const reload = setInterval(buscarCotacoes, 10 * 60 * 1000);
  ...
  return (
    <div className="cotacoes-banner">
      <span className="cotacoes-banner-tag">💱 Cotações</span>
      <div className="cotacoes-track-wrapper">
        <div className="cotacoes-track" style={{ animationDuration: `${duracaoSeg}s` }}>
          {itensMarquee.map((c, i) => (
            <div className="cotacao-item" key={i}>
              <span className="cotacao-item-label">{c.label}</span>
              <span className="cotacao-item-valor">{c.valor}</span>
            </div>
          ))}
        </div>
      </div>
...
function App() {
  ...
  const selectOption = (option) => {
    const responses = {
      services: { title: '📦 Nossos Serviços', text: 'Oferecemos soluções completas em Transporte RodoFluvial, Distribuição de Cargas, Armazenagem e Exportação.', whatsappMsg: 'Olá! Gostaria de conhecer melhor os serviços da JF Organização.' },
      quote:    { title: '💰 Solicitar Orçamento', text: 'Vamos elaborar um orçamento personalizado para sua empresa!', whatsappMsg: 'Olá! Gostaria de solicitar um orçamento.' },
      export:   { title: '🌍 Exportação', text: 'Cuidamos de todo o processo de exportação com assessoria completa em comércio internacional.', whatsappMsg: 'Olá! Preciso de informações sobre exportação.' },
      other:    { title: '❓ Outras Dúvidas', text: 'Nossa equipe está pronta para esclarecer suas dúvidas!', whatsappMsg: 'Olá! Tenho algumas dúvidas sobre os serviços.' }
    };
    setChatResponse(responses[option]);
  };

  const resetChat = () => setChatResponse(null);
  ...
  return (
    <div className="App">
      ...
      {/* ── Cotações de Moedas & Commodities ── */}
      <CotacoesBanner />
      ...
      <section className="services" id="servicos">
        <div className="container">
          ...
          <div className="services-grid">
            <div className="service-card" style={{ backgroundImage: `linear-gradient(rgba(0,20,60,0.30),rgba(0,70,150,0.30)),url(${image1})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
              <div className="service-card-content">
                ...
                <a href={waLink('service1')} target="_blank" rel="noopener noreferrer" className="service-whatsapp-btn">💬 Falar com especialista</a>
              </div>
            </div>

            <div className="service-card" style={{ backgroundImage: `linear-gradient(rgba(0,20,60,0.30),rgba(0,70,150,0.30)),url(${image4})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
              <div className="service-card-content">
                ...
                <a href={waLink('service2')} target="_blank" rel="noopener noreferrer" className="service-whatsapp-btn">💬 Falar com especialista</a>
              </div>
            </div>

            <div className="service-card" style={{ backgroundImage: `linear-gradient(rgba(0,20,60,0.30),rgba(0,70,150,0.30)),url(${image3})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
              <div className="service-card-content">
                ...
                <a href={waLink('service3')} target="_blank" rel="noopener noreferrer" className="service-whatsapp-btn">💬 Falar com especialista</a>
              </div>
            </div>

            <div className="service-card" style={{ backgroundImage: `linear-gradient(rgba(0,20,60,0.30),rgba(0,70,150,0.30)),url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
              <div className="service-card-content">
                ...
                <a href={waLink('service4')} target="_blank" rel="noopener noreferrer" className="service-whatsapp-btn">💬 Falar com especialista</a>
              </div>
            </div>

            <div className="service-card" style={{ backgroundImage: `linear-gradient(rgba(0,20,60,0.30),rgba(0,70,150,0.30)),url(${image5})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
              <div className="service-card-content">
                ...
                <a href={waLink('service5')} target="_blank" rel="noopener noreferrer" className="service-whatsapp-btn">💬 Falar com especialista</a>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="map-container-enhanced">
              ...
              >
                ...
                {locations.map((location, index) => (
                  <Marker key={index} position={location.coords} icon={location.type === 'matriz' ? matrizIcon : filialIcon}>
                    <Popup>
                      <div style={{ textAlign: 'center', padding: '0.5rem' }}>
                        <strong style={{ fontSize: '1.1rem', color: location.type === 'matriz' ? '#dc2626' : '#2563eb', display: 'block', marginBottom: '0.5rem' }}>
                          {location.type === 'matriz' ? '🏢 ' : '🏪 '}{location.name}
                        </strong>
      ...
      <section className="whatsapp-cta">
        <div className="container">
          ...
          <a href="https://wa.me/5592992091329?text=Olá,%20gostaria%20de%20solicitar%20um%20orçamento" target="_blank" rel="noopener noreferrer" className="btn-whatsapp">
        ...
        <div className="container">
          ...
          <div style={{ maxWidth: '600px', margin: '0 auto' }}>
            <div className="contact-form">
              <h3 style={{ color: 'var(--primary)', marginBottom: '1.5rem', fontSize: '1.8rem' }}>Solicite seu Orçamento</h3>
              <p style={{ marginBottom: '2rem', color: '#555', fontSize: '1.1rem' }}>Entre em contato conosco e receba atendimento personalizado para suas necessidades de exportação.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <a href="https://wa.me/5592992091329?text=Olá,%20gostaria%20de%20solicitar%20um%20orçamento" target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', textDecoration: 'none' }}>
                  <span style={{ fontSize: '1.5rem' }}>💬</span><span>WhatsApp - Comercial</span>
                </a>
                <a href="mailto:export@jforganizacao.com.br" className="btn btn-secondary" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', textDecoration: 'none', background: 'var(--secondary)', color: 'white' }}>
                  <span style={{ fontSize: '1.5rem' }}>✉️</span><span>Email - Comercial Exportação</span>
                </a>
              </div>
              <div style={{ marginTop: '2rem', padding: '1.5rem', background: 'var(--light)', borderRadius: '10px', borderLeft: '4px solid var(--accent)' }}>
                <p style={{ color: '#555', marginBottom: '0.5rem' }}><strong>📧 Contato Comercial Exportação:</strong></p>
      ...
      <footer style={{ backgroundImage: `url(${logoImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
        <div className="footer-content">
          ...
          <div className="footer-section">
            ...
            <ul>
              ...
              <li><a href={waLink('service5')} target="_blank" rel="noopener noreferrer">Transporte Internacional</a></li>
            </ul>
          </div>
          <div className="footer-section">
            <h3>Empresa</h3>
          ...
          <div className="footer-section">
            ...
            <div className="footer-social-icons">
              <a href="https://www.linkedin.com" target="_blank" rel="noopener noreferrer" className="footer-social-link" title="LinkedIn"><img src={linkedinIcon} alt="LinkedIn" /></a>
              <a href="https://www.facebook.com" target="_blank" rel="noopener noreferrer" className="footer-social-link" title="Facebook"><img src={facebookIcon} alt="Facebook" /></a>
              <a href="https://www.instagram.com/jforganizacao?igsh=MTh2cDk0Z2kzcmlzbg==" target="_blank" rel="noopener noreferrer" className="footer-social-link" title="Instagram"><img src={instagramIcon} alt="Instagram" /></a>
              <a href="https://wa.me/5592992091329" target="_blank" rel="noopener noreferrer" className="footer-social-link" title="WhatsApp Business"><img src={whatsappIcon} alt="WhatsApp" /></a>
            </div>
          </div>
        </div>
        ...
        {!chatOpen ? (
          ...
          <div id="chat-box">
            <div style={{ background: 'var(--primary)', color: 'white', padding: '1rem', borderRadius: '10px 10px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => { setChatOpen(false); resetChat(); }} style={{ background: 'none', border: 'none', color: 'white', fontSize: '1.5rem', cursor: 'pointer', padding: 0, width: '30px', height: '30px' }}>&times;</button>
            </div>
            <div style={{ padding: '1.5rem', flex: 1, overflowY: 'auto' }}>
              {!chatResponse ? (
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                    <a href={`https://wa.me/5592992091329?text=${encodeURIComponent(chatResponse.whatsappMsg)}`} target="_blank" rel="noopener noreferrer"
                      style={{ background: '#25D366', color: 'white', padding: '1rem 1.5rem', borderRadius: '10px', textDecoration: 'none', textAlign: 'center', fontWeight: '600', fontSize: '1.05rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                    ...
                    <button onClick={resetChat} style={{ background: 'var(--light)', color: 'var(--dark)', padding: '0.9rem', borderRadius: '10px', border: '1px solid #ddd', cursor: 'pointer', fontWeight: '600' }}>
                      ← <span>Voltar</span>
                    </button>
                  </div>
                </div>
